...
from ..article_service import ARTICLE_SNAPSHOT_FIELDS, apply_article_payload, article_dict, content_hash, sync_search_shadow
from ..database import get_session
from ..models import Article, ArticleColumn, ArticleDraft, ArticleTag, ContentEntry, ContentVersion, KnowledgeColumn, KnowledgeColumnNode
from ..schemas import ArticleAutosave, ArticleWrite, KnowledgeColumnWrite
...
def list_articles(
    ...
    query = select(Article).order_by(Article.updated_at.desc(), Article.id.desc())
    if status:
...
def delete_article(
    ...
    if not article:
        raise HTTPException(status_code=404, detail="Article not found")
    delete_draft(session, article.id)
    for model in (ArticleTag, ArticleColumn):
        for link in session.scalars(select(model).where(model.article_id == article.id)):
            session.delete(link)
    for version in session.scalars(
        select(ContentVersion).where(ContentVersion.entity_type == "article", ContentVersion.entity_id == article.id)
    ):
...
def list_columns(
    ...
    columns = session.scalars(select(KnowledgeColumn).order_by(KnowledgeColumn.sort_order, KnowledgeColumn.name))
...
def delete_column(
    ...
    if not column:
        raise HTTPException(status_code=404, detail="Knowledge column not found")
    for link in session.scalars(select(ArticleColumn).where(ArticleColumn.column_id == column.id)):
        session.delete(link)
    for link in session.scalars(select(KnowledgeColumnNode).where(KnowledgeColumnNode.column_id == column.id)):
        session.delete(link)
